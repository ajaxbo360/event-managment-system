...
class EventController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $events = $query->orderBy('date_time', 'asc')
            ->get()
            ->map(function ($event) use ($request) {
                $userRegistration = $event->users()
                ...
                return [
                    ...
                    'available_waitlist_spots' => $event->availableWaitlistSpots(),
                    'is_full' => $event->isFull(),
                    'is_waitlist_full' => $event->isWaitlistFull(),
                    'registration_status' => $userRegistration ? $userRegistration->pivot->status : null,
                    'is_joined' => $userRegistration !== null,
                ];
            });
        ...
        $events = $user->events()
            ...
            ->map(function ($event) {
                return [
                    ...
                    'location' => $event->location,
                    'capacity' => $event->capacity,
                    'registration_status' => $event->pivot->status,
